...
class P2P {
	...
	public static function bind(notifier:Notifier<Dynamic>, id:String, transmit:Bool, receive:Bool):Void {
		if (transmit)
			transmitters.set(id, new WindowTransmitter(notifier, id));
		if (receive)
			receivers.set(id, new WindowReceiver(notifier, id));
	}
...
class P2P {
	...
	public static function bind(notifier:Notifier<Dynamic>, id:String, transmit:Bool, receive:Bool):Void {
		if (transmit)
			transmitters.set(id, new Transmitter(notifier, id));
		if (receive)
			receivers.set(id, new Receiver(notifier, id));
	}
	...
	static private function UpdateConnection() {
		...
		if (type.value == P2PConnectionType.CLOUD) {
			...
			if (idAddress.value == null) {
				...
				peerObject.connect("rtmfp://" + idAddress.value);
