...
class TransformObject implements ITransformObject implements IInternetTransformObject {
	...
	private function OnStageResize() {
		if (transformAnchor == true || transformScale == true || transformAlign == true) {
			...
			if (transformAnchor == true)
				transformationMatrix.translate(_anchorX, _anchorY);
			if (transformScale == true)
				transformationMatrix.scale(_scaleContainerScaleX, _scaleContainerScaleY);
			if (transformAlign == true)
				transformationMatrix.translate(_alignX, _alignY);
	...
	private function calcAlignment():Void {
		...
		var _alignX:Float = 0;
		var _alignY:Float = 0;
		if (layoutSettings.align.fraction.x != null)
			_alignX += (layoutSettings.align.fraction.x * frameWidth);
		if (layoutSettings.align.fraction.y != null)
			_alignY += (layoutSettings.align.fraction.y * frameHeight);
		if (layoutSettings.align.pixels.x != null)
			_alignX += layoutSettings.align.pixels.x;
		if (layoutSettings.align.pixels.y != null)
			_alignY += layoutSettings.align.pixels.y;
	...
	private function calcAnchor():Void {
		...
		if (fraction.x == null)
			fraction.x = layoutSettings.align.fraction.x;
		if (fraction.y == null)
			fraction.y = layoutSettings.align.fraction.y;
		...
		if (layoutSettings.updateBounds || anchorBounds == null) {
			...
			if (anchorBounds == null)
				anchorBounds = layoutContainer.bounds;
		}
		if (anchorBounds != null) {
			if (fraction.x != null)
				_anchorX = (fraction.x * -anchorBounds.width);
			if (fraction.y != null)
				_anchorY = (fraction.y * -anchorBounds.height);
