...
class ScrollManager {
	...
	public function addTouchArea(scrollTouchArea:DisplayObject) {
		...
		TouchManager.add(scrollTouchArea).setBegin(OnBegin).setMove(OnMove).setEnd(OnRelease);
	...
	function OnMove(touch:Touch) {
		...
		if (target.y > initPosition.y)
			target.y = initPosition.y;
		if (target.y < initPosition.y - scrollLength)
			target.y = initPosition.y - scrollLength;
