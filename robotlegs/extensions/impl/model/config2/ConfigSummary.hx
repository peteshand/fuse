...
class ConfigSummary {
	...
	function createCombinedLocal() {
		if (localConfigData != null)
			copyFromTo(localConfigData, combinedConfigData, true);
		if (seedConfigData != null)
			copyFromTo(seedConfigData, combinedConfigData);
	...
	function copyFromTo(fromConfigData:ConfigData, toConfigData:ConfigData, copyFlags:Bool = false) {
		if (toConfigData._filename == null)
			toConfigData._filename = fromConfigData._filename;
		if (toConfigData._location == null)
			toConfigData._location = fromConfigData._location;
		if (toConfigData._name == null)
			toConfigData._name = fromConfigData._name;

		if (copyFlags && fromConfigData.flags != null) {
			for (i in 0...fromConfigData.flags.length) {
				if (toConfigData.flags == null)
					toConfigData.flags = [];
			...
			for (i in 0...fromConfigData.props.length) {
				...
				if (toConfigData.props == null) {
					...
					if (alreadyAdded != -1) {
						...
						if (overrideProp)
							toConfigData.props[alreadyAdded] = fromConfigData.props[i];
					} else {
