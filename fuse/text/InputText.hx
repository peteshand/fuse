...
class InputText extends TextField
{
    ...
    {
        hitArea.width = width;
        if (textHeight == 0) hitArea.height = height + 10;
        else hitArea.height = textHeight + 10;
    }
    ...
    function onKeyPress()
    {
        if (stage.focus.value != this) return;
        ...
        if (keyCode == Key.RIGHT){
            ...
            if (text.length == 0 || _charIndex == 0) return;
            ...
            for (i in 0...ignoreKeyCodes.length) {
                if (/*charCode == ignoreKeyCodes[i] ||*/ keyCode == ignoreKeyCodes[i]) return;
            }
    ...
    override function set_text(value:String):String
	{
        ...
        if (charIndex.value < 0) charIndex.value = 0 + offset;
        else charIndex.value += offset;
        return value;
    }
