...
import fuse.text.textDisplay.textures.Texture;
...
class Scale9Image extends Sprite {
	...
	public function new(texture:Texture, centerRect:Rectangle):Void {
		...
		_br = new Image(Texture.fromTexture(texture,
			new Rectangle(_grid.x
				+ _grid.width, _grid.y
				+ _grid.height, texture.width
				- (_grid.x + _grid.width), texture.height
