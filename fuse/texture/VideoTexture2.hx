...
abstract VideoTexture2(AbstractTexture) to Int from Int 
{
	...
	{
		var baseVideoTexture:BaseVideoTexture = new BaseVideoTexture(AbstractTexture.textureIdCount++, width, height, netStream, queUpload, onTextureUploadCompleteCallback);
...
class BaseVideoTexture extends BaseTexture
{
	...
	public function new(textureId:Int, ?width:Int, ?height:Int, netStream:NetStream, queUpload:Bool=false, onTextureUploadCompleteCallback:Void -> Void = null) 
	{
		this.netStream = netStream;
		this.netStream.client = { onMetaData: onMetaData };
		trace("BaseVideoTexture: " + textureId);
		super(textureId, width, height, queUpload, onTextureUploadCompleteCallback, false);
