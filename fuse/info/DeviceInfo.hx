...
class DeviceInfo {
	...
	static function __init__():Void {
		...
		if (manufacturer.indexOf("windows") != -1 || manufacturer.indexOf("osx") != -1)
			isMobile = false;
		else
			isMobile = true;
		...
		if (isMobile) {
			...
			if ((physicalScreenSize > tabletScreenMinimumInches))
				isTablet = true;
			else
				isPhone = true;

			if (physicalScreenSize < 7)
				isCardboardCompatible = true;
			else
				isCardboardCompatible = false;
		}
	}
