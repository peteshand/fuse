...
private class DestRegister {
	...
	public function toGLSL (useMask:Bool = true):String {
		...
		if (type == RegisterType.OUTPUT) {
			...
			str = AGALConverterTest.prefixFromType (type, programType) + n;
...
class RegisterMap {
	...
	public function addSR (sr:SourceRegister, usage:RegisterUsage, offset:Int = 0):Void {
		...
		if (sr.d != 0) {
			...
			add (sr.itype, AGALConverterTest.prefixFromType (sr.itype, sr.programType) + sr.n, sr.n, RegisterUsage.VECTOR_4);
			add (sr.type, AGALConverterTest.prefixFromType (sr.type, sr.programType) + sr.o, sr.o, RegisterUsage.VECTOR_4_ARRAY);
			return;
...
private class SamplerRegister {
	...
	public function toGLSL ():String {
		...
		var str = AGALConverterTest.prefixFromType (type, programType) + n;
		return str;
...
private class SourceRegister {
	...
	public function toGLSL (emitSwizzle:Bool = true, offset:Int = 0):String {
		...
		var str = AGALConverterTest.prefixFromType (type, programType);
		...
		if (d == 0) {
			...
			var indexRegister = AGALConverterTest.prefixFromType (itype, programType) + this.n + "." + indexComponent;
