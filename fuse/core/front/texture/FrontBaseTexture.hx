...
class FrontBaseTexture implements IFrontTexture {
	...
	public function new(width:Int, height:Int, queUpload:Bool = true, p2Texture:Bool = true, ?_textureId:Null<TextureId> = null,
		...
		if (_textureId == null) {
			...
			if (FrontBaseTexture.textureIdCount <= _textureId)
				FrontBaseTexture.textureIdCount = _textureId + 1;
		}
		...
		if (_objectId == null) {
			...
			// this.textureId = _textureId;
			if (FrontBaseTexture.objectIdCount <= _objectId)
				FrontBaseTexture.objectIdCount = _objectId + 1;
	...
	public function createNativeTexture() {
		...
		textureData.textureBase = textureData.nativeTexture = Textures.context3D.createTexture(textureData.p2Width, textureData.p2Height,
			Context3DTextureFormat.BGRA, false, 0);
		#if air
		try {
	...
	public function createSubTexture(offsetU:Float, offsetV:Float, scaleU:Float, scaleV:Float):FrontSubTexture {
		var texture = new FrontSubTexture(offsetU, offsetV, scaleU, scaleV, this);
		return texture;
	}
}
