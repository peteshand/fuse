...
class CoreDisplayObject {
	...
	inline function checkUpdates() {
		if (Graphics.parent.updateAny)
			updateAny = true;
		if (updateAny) {
			if (Graphics.parent.updatePosition)
				updatePosition = true;
			if (Graphics.parent.updateRotation)
				updateRotation = true;
			if (Graphics.parent.updateColour)
				updateColour = true;
			if (Graphics.parent.updateVisible)
				updateVisible = true;
			if (Graphics.parent.updateAlpha)
				updateAlpha = true;
			if (Graphics.parent.updateTexture)
				updateTexture = true;
		}
	...
	public function onPress(index:Int):Touch {
		return getTouch(_onPresses, index, TouchType.PRESS);
	}

	public function onRelease(index:Int):Touch {
		return getTouch(_onRelease, index, TouchType.RELEASE);
	}

	public function onMove(index:Int):Touch {
		return getTouch(_onMove, index, TouchType.MOVE);
	}

	public function onOver(index:Int):Touch {
		return getTouch(_onOver, index, TouchType.OVER);
	}

	public function onOut(index:Int):Touch {
		return getTouch(_onOut, index, TouchType.OUT);
	}
	...
	public function absoluteVis():Bool {
		if (visible == false)
			return false;
		if (parent != null)
			return parent.absoluteVis();
		else if (this.displayType != DisplayType.STAGE)
			return false;
	...
	public function getTriangleSum(x:Float, y:Float) {
		var t1:Float = triangleArea(x, y, quadData.bottomLeftX, quadData.bottomLeftY, quadData.topLeftX, quadData.topLeftY);
		var t2:Float = triangleArea(x, y, quadData.topLeftX, quadData.topLeftY, quadData.topRightX, quadData.topRightY);
		var t3:Float = triangleArea(x, y, quadData.topRightX, quadData.topRightY, quadData.bottomRightX, quadData.bottomRightY);
		var t4:Float = triangleArea(x, y, quadData.bottomRightX, quadData.bottomRightY, quadData.bottomLeftX, quadData.bottomLeftY);
