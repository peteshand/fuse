...
class DisplayList {
	...
	inline function getDisplayFromPool(displayType:Int):CoreDisplayObject {
		switch displayType {
			case DisplayType.STAGE:
				return new CoreStage();
			case DisplayType.SPRITE:
				return Pool.sprites.request();
			case DisplayType.SCALE9_IMAGE:
				return Pool.sprites.request();
			case DisplayType.DISPLAY_OBJECT:
				return Pool.displayObjects.request();
			case DisplayType.IMAGE:
				return Pool.images.request();
			case DisplayType.MOVIECLIP:
				return Pool.movieclips.request();
			case DisplayType.QUAD:
				return Pool.quads.request();
			default:
				return null;
		}
	}
	...
	inline function releaseDisplayToPool(display:CoreDisplayObject) {
		if (display.displayType == DisplayType.SPRITE)
			Pool.sprites.release(untyped display);
		if (display.displayType == DisplayType.DISPLAY_OBJECT)
			Pool.displayObjects.release(untyped display);
		if (display.displayType == DisplayType.IMAGE)
			Pool.images.release(untyped display);
		if (display.displayType == DisplayType.QUAD)
			Pool.quads.release(untyped display);
	}
