...
class AtlasSheet {
	...
	function initAvailablePartitions() {
		...
		availablePartition.push( // Pool.atlasPartitions2.request()
			new AtlasPartition().init(1, 1, Fuse.MAX_TEXTURE_SIZE - 2, Fuse.MAX_TEXTURE_SIZE - 2));
	}
		...
		for (i in 0...availablePartition.length) {
			...
			if (successfulPlacement) {
				...
				if (partition.bottomPartition != null) {
					if (added)
						availablePartition.push(partition.bottomPartition);
					else
						availablePartition[i] = partition.bottomPartition;
					added = true;
				}
				activePartitions.push(partition);
				if (!added)
	...
	function noChanges(p:GcoArray<AtlasPartition>) {
		...
		var count:Int = 0;
		for (j in 0...p.length) {
			if (p[j].coreTexture.textureHasChanged)
				return false;
			if (p[j].lastFramePairPartition != null)
				count++;
		}
		if (count == p.length)
			return true;
