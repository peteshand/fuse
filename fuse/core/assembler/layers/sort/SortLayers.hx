...
class SortLayers {
	...
	static public function build() {
		if (!GenerateLayers.layersGenerated)
			return;

		if (cacheLayerImages.length == 0)
			init();
		clear();
	...
	static inline function SetActiveBasedOnPosition() {
		...
		for (i in 0...GenerateLayers.layers.length) {
			if (count >= LayerCacheBuffers.numBuffers)
				break;
			if (GenerateLayers.layers[i].updateAll)
				break;
	...
	static private function CombineInactive() {
		...
		for (k in 0...newDirectLayers.length)
			GenerateLayers.layers.push(newDirectLayers[k]);
	}
}
