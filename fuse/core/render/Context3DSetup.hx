...
class Context3DSetup {
	...
	public function new() {
		...
		if (PlayerVersion.majorMinor >= 16.0)
			profiles.push(Context3DProfile.STANDARD);
		if (PlayerVersion.majorMinor >= 14.0)
			profiles.push(Context3DProfile.STANDARD_CONSTRAINED);
		if (PlayerVersion.majorMinor >= 11.8)
			profiles.push(Context3DProfile.BASELINE_EXTENDED);
		if (PlayerVersion.majorMinor >= 11.4)
