...
class Buffer {
	...
	inline function updateIndices() {
		indexbuffer.uploadFromByteArray(indicesData, 0, 0, Buffer.INDICES_PER_QUAD * bufferSize);
	}
	...
	inline function updateVertices(start:Int, length:Int) {
		...
		vertexbuffer.uploadFromByteArray(SharedMemory.memory, VertexData.BYTES_PER_ITEM * (BatchRenderer.quadCount + start), Buffer.VERTICES_PER_QUAD * start,
