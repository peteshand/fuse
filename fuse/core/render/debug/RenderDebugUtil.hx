...
class RenderDebugUtil {
	...
	public static function batchDebug(batchData:IBatchData) {
		...
		trace([
			batchData.textureId1,
			batchData.textureId2,
			batchData.textureId3,
			batchData.textureId4,
			batchData.textureId5,
			batchData.textureId6,
			batchData.textureId7,
			batchData.textureId8
		]);
		...
		for (j in 0...numItems) {
			...
			for (k in 0...Buffer.VERTICES_PER_QUAD) {
				...
				if (k == 0) {
					trace([
						INDEX_Texture,
						Math.floor(1024 * (1 + INDEX_X)),
						Math.floor(2048 - ((1 + INDEX_Y) * 1024))
					]);
				}
